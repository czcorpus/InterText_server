...
# Initialization (constructor)
	function InterText () {
		...
		if (!$this->_spojeni = @MySQL_Connect($DB_SERVER,$DB_USER,$DB_PASSWORD)) 
			$this->_fail("Cannot connect to database: ".mysql_error());
		if (!$result = mysql_select_db($DB_DATABASE))
			$this->_fail("Cannot open database '$DB_DATABASE': ".mysql_error());
		if (!$result = mysql_query("SET CHARACTER SET utf8"))
				$this->_fail("Cannot set encoding UTF-8: ".mysql_error());
		$this->autor = array();
		if (!$DISABLE_FULLTEXT) {
      if (!$dbresult = mysql_query("SELECT VERSION()>5.5"))
        die("Cannot access database: ".mysql_error());
      $ret = mysql_fetch_row($dbresult);
      if (!$ret[0]) {
...
# Import new XML from file
    ...
    if (!$dbresult = mysql_query("START TRANSACTION")) {
      $this->_failure("Cannot start transaction: ".mysql_error());
      return FALSE;
    }
    ...
    if ($verid===FALSE)
      return FALSE;
		if (!$dbresult = mysql_query("UPDATE `{$txtid}_elements` SET txtver_id='$verid' WHERE (id='$root_id')")) {
			$this->_failure("Cannot access database: ".mysql_error());
			return FALSE;
    }
		$stat = FALSE;
    ...
    $_ERROR = '';
      ...
      if (!$dbresult = mysql_query("COMMIT")) {
        $this->_failure("Cannot commit transaction: ".mysql_error());
        return FALSE;
      }
		}
		$_ERROR = '';
		...
		if (!$dbresult = mysql_query($query))
			$this->_fail("Cannot check database: ".mysql_error()." Query: $query\n");
		if (!mysql_num_rows($dbresult)) {
			...
			if (!$dbresult = mysql_query($query))
				$this->_fail("Cannot check database: ".mysql_error()." Query: $query\n");
			if (!mysql_num_rows($dbresult)) $this->set_uniq_ids($txtid,$verid,true);
		}
		return TRUE;
	}
...
# Export document
	function export_document($txt,$id,$cor_aid=0,$format='xml',$maxcor_stat=STATUS_AUTOMATIC) {
		...
		if ($segs) {
			...
			if (!$dbresult = mysql_query($query))
				$this->_fail("Cannot access database: ".mysql_error());
			while ($ret = mysql_fetch_assoc($dbresult)) { $seg[$ret['id']] = $ret['pos']; $lastid=$ret['id']; }
			...
			if (!$dbresult = mysql_query($query))
				$this->_fail("Cannot access database: ".mysql_error());
			while ($ret = mysql_fetch_assoc($dbresult)) $corr[$ret['id']] = $ret['corresp'];
		}
...
# Import alignment
    ...
    if (!$dbresult = mysql_query("START TRANSACTION")) {
      ...
      return FALSE;
		...
		if (!$dbresult = mysql_query("SELECT id,element_id FROM `{$txt}_elements` WHERE (txtver_id='{$al['ver1_id']}' AND ($alignable1)) ORDER BY txt_position")) {
			$this->_failure("Cannot access database: ".mysql_error());
      if ($report) print("$_ERROR\n");
      return FALSE;
    }
		while ($ret = mysql_fetch_assoc($dbresult)) { $v1idx[$ret['element_id']]= $ret['id']; $v1list[]= $ret['element_id']; }
		if (!$dbresult = mysql_query("SELECT id,element_id FROM `{$txt}_elements` WHERE (txtver_id='{$al['ver2_id']}' AND ($alignable2)) ORDER BY txt_position")) {
			$this->_failure("Cannot access database: ".mysql_error());
      if ($report) print("$_ERROR\n");
      return FALSE;
    }
		while ($ret = mysql_fetch_assoc($dbresult)) { $v2idx[$ret['element_id']]= $ret['id']; $v2list[]= $ret['element_id']; }
		$position = 0;
...
# Export alignment
	function export_alignment($aid,$format='xml',$maxstatus=STATUS_PLAIN) {
...
# Plain alignment 1:1
	function plain_alignment($aid,$report=TRUE) {
		...
		mysql_query("CHECK TABLE `{$txt}_links`");
    if (!$dbresult = mysql_query("START TRANSACTION")) {
      $this->_failure("Cannot start transaction: ".mysql_error());
      return FALSE;
    }
		# Get list of unaligned elements from text version 1
		if (!$dbresult = mysql_query("SELECT id FROM `{$txt}_elements` WHERE (txtver_id='$v1' AND ($alignable1) AND id NOT IN (SELECT element_id FROM `{$txt}_links` WHERE alignment_id='$aid' AND version_id='$v1')) ORDER BY txt_position")) {
			$this->_failure("Cannot access database: ".mysql_error());
      return FALSE;
    }
		$v1_ids = array(); $v2_ids = array();
		if (mysql_num_rows($dbresult))
			while ($ret = mysql_fetch_assoc($dbresult)) $v1_ids[]=$ret['id'];
		# Get list of unaligned elements from text version 2
		if (!$dbresult = mysql_query("SELECT id FROM `{$txt}_elements` WHERE (txtver_id='$v2' AND ($alignable2) AND id NOT IN (SELECT element_id FROM `{$txt}_links` WHERE alignment_id='$aid' AND version_id='$v2')) ORDER BY txt_position")) {
			$this->_failure("Cannot access database: ".mysql_error());
      return FALSE;
    }
		if (mysql_num_rows($dbresult))
			while ($ret = mysql_fetch_assoc($dbresult)) $v2_ids[]=$ret['id'];
		# Nothing more to do here? ...return
		if (count($v1_ids)==0 AND count($v2_ids)==0) {
			if ($report) { print "Process: Nothing to align.\n"; flush(); ob_flush(); }
      if (!$dbresult = mysql_query("COMMIT")) {
        $this->_failure("Cannot commit transaction: ".mysql_error()); // Is this a problem...?
      }
			return TRUE;
		}
		# Otherwise align the rest 1:1
		# first: get the position of the last alignment link
		if (!$dbresult = mysql_query("SELECT max(position) as last FROM `{$txt}_links` WHERE (alignment_id='$aid')")) {
			$this->_failure("Cannot access database: ".mysql_error());
      return FALSE;
    }
		if (!mysql_num_rows($dbresult)) $position=0;
		else {
			$ret = mysql_fetch_assoc($dbresult);
			$position = $ret['last'];
		}
...
# Delete document from database
	function delete_document($txt,$id) {
		$txtver = $this->txtver_by_id($id);
		if (!$dbresult = mysql_query("SELECT id FROM alignments WHERE (ver1_id=$id OR ver2_id=$id)"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (mysql_num_rows($dbresult))
			$this->_fail("Cannot delete document - it is used by one or more alignments!");
		# Now delete the record for the version
		if (!$dbresult = mysql_query("DELETE FROM `{$txt}_elements` WHERE (txtver_id=$id)"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (!$dbresult = mysql_query("DELETE FROM `{$txt}_changelog` WHERE (txtver_id=$id)"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (!$dbresult = mysql_query("DELETE FROM versions WHERE (text_id=$txt AND id=$id)"))
			$this->_fail("Cannot access database: ".mysql_error());
		# Any more versions of this text? If not, delete the text record too
		if (!$dbresult = mysql_query("SELECT id FROM versions WHERE text_id=$txt"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (!mysql_num_rows($dbresult)) {
			if (!$dbresult = mysql_query("DELETE FROM texts WHERE id=$txt"))
				$this->_fail("Cannot access database: ".mysql_error());
			if (!$dbresult = mysql_query("DROP TABLE `{$txt}_elements`, `{$txt}_links`, `{$txt}_changelog`"))
				$this->_fail("Cannot access database: ".mysql_error());
			if (!$dbresult = mysql_query("DROP TABLE IF EXISTS `{$txt}_align_changelog`"))
				$this->_fail("Cannot access database: ".mysql_error());
		}
	}
...
# Get info about document version by name
	function txtver_info($txt,$version) {
		...
		if (!$dbresult = mysql_query("SELECT * FROM versions WHERE text_id=$txt AND version_name='$version'"))
			return FALSE; #$this->_fail("Cannot access database: ".mysql_error());
		if (!mysql_num_rows($dbresult))
			return FALSE;
		else {
			$res = mysql_fetch_assoc($dbresult);
			return $res;
		}
	}
...
# Get info about document version by filename
	function txtver_by_filename($filename) {
		...
		if (!$dbresult = mysql_query("SELECT * FROM versions WHERE filename='$filename'"))
			return FALSE; #$this->_fail("Cannot access database: ".mysql_error());
		if (!mysql_num_rows($dbresult))
			return FALSE;
		else {
...
# Get info about document version by its id
	function txtver_by_id($id) {
		...
		if (!$dbresult = mysql_query("SELECT v.*, t.name as text_name, t.id as text_id FROM versions v, texts t WHERE v.text_id=t.id AND v.id='$id'")) {
			$this->_failure("Cannot access database: ".mysql_error());
			return FALSE;
		}
		if (!mysql_num_rows($dbresult)) {
...
# Get name of text by id
	function textname_by_id($id) {
		if (!$dbresult = mysql_query("SELECT * FROM texts WHERE id=$id"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (mysql_num_rows($dbresult)) {
			$res = mysql_fetch_assoc($dbresult);
			return $res['name'];
...
# Register text or get text-id
    ...
    global $DISABLE_FULLTEXT;
		...
		#if (!$dbresult = mysql_query("SHOW TABLES LIKE '{$name}_versions'"))
		if (!$dbresult = mysql_query("SELECT * FROM texts WHERE name='$name'"))
			$this->_fail("Cannot check tables in database: ".mysql_error());
		if (mysql_num_rows($dbresult)) {
			$res = mysql_fetch_assoc($dbresult);
			return $res['id'];
		} else {
        ...
        if (!$dbresult = mysql_query("START TRANSACTION")) {
          ...
          return FALSE;
				...
				if (!$dbresult = mysql_query("INSERT INTO texts (name) VALUES ('$name')")) {
					$this->_failure("Cannot insert new text into database: ".mysql_error());
					return FALSE;
				}
...
/*				if (!$dbresult = mysql_query($table_versions))
					$this->_fail("Cannot create new text in the database: ".mysql_error());*/
				if (!$dbresult = mysql_query("SET storage_engine=InnoDB")) {
					$this->_failure("Cannot set default storage engine to InnoDB: ".mysql_error());
          return FALSE;
        }
				if (!$dbresult = mysql_query($table_elements)) {
					$this->_failure("Cannot create new text elements table in the database: ".mysql_error().". Are you sure you have a supported version of MySQL (>=5.6)?");
          if (!$dbresult = mysql_query("DELETE FROM texts WHERE id=$txtid")) { // just to be sure for bad upgraders
            $this->_failure("Cannot insert new text into database: ".mysql_error());
            return FALSE;
          }
          return FALSE;
        }
				if (!$dbresult = mysql_query($table_links)) {
					$this->_failure("Cannot create new text link table in the database: ".mysql_error());
          return FALSE;
        }
				if (!$dbresult = mysql_query($table_changelog)) {
					$this->_failure("Cannot create new text changelog table in the database: ".mysql_error());
          return FALSE;
        }
        if (!$dbresult = mysql_query("COMMIT")) {
          $this->_failure("Cannot commit transaction: ".mysql_error());
          return FALSE;
				...
				//if (!$dbresult = mysql_query($table_align_changelog))
				//	$this->_fail("Cannot create new alignment changelog table in the database: ".mysql_error());
				return $txtid;
			}
		}
	}
...
# Register new text version
	function insert_version($txtid,$name,$root_id,$text_elements,$filename='') {
		...
		if (!$dbresult = mysql_query("INSERT INTO versions (text_id,version_name,root_id, text_elements, filename) VALUES ($values)")) {
			$this->_failure("Cannot insert new text into database: ".mysql_error());
			return FALSE;
		}
...
# Test existence of alignment
	function alignment_exists($txt_id,$v1_id,$v2_id) {
		...
		if (!$dbresult = mysql_query($query))
			$this->_fail("Cannot access database: ".mysql_error()." Query: $query\n");
		if (mysql_num_rows($dbresult))
			return true;
		else
			return false;
...
# Insert new aligenment
	function insert_alignment($txt_id,$v1_id,$v2_id,$method='',$profile='',$resp=0,$editor=0,$edit=0) {
		...
		if (!$dbresult = mysql_query($query))
			$this->_fail("Cannot insert new alignment into database: ".mysql_error()." Query: $query");
		...
		if (!$dbresult = mysql_query($query))
			$this->_fail("Cannot reset c_chstruct permission: ".mysql_error()." Query: $query");
		return $mysqlid;
	}
...
# Add new link item
	function _new_link_item($txt,$aid,$vid,$eid,$position,$status,$mark=0) {
		...
		if (!$dbresult = mysql_query("INSERT INTO `{$txt}_links` (alignment_id,version_id,element_id,position,status,mark) VALUES ($values)")) {
			$this->_failure("Cannot insert new link into database: ".mysql_error());
			return FALSE;
		}
		return mysql_insert_id();
	}
...
# Insert element into database
	function _insert_element($txt,$txtverid,$txt_position,$parent_id,$position,$name,$eid='',$attributes='',$contents='NULL') {
		$name = "'".mysql_real_escape_string($name)."'";
		if ($contents!='NULL') $contents = "'".mysql_real_escape_string($contents)."'";
		$attributes = mysql_real_escape_string($attributes);
		$eid = mysql_real_escape_string($eid);
/*		if (!$dbresult = mysql_query("SELECT id FROM elements WHERE parent='$parent_id'"))
			$this->_fail("Cannot count elements in database: ".mysql_error());
		$position = mysql_num_rows($dbresult)+1;*/
		$values = "$txtverid,$txt_position,$parent_id,$position,$name,'$eid','$attributes',$contents";
		if (!$dbresult = mysql_query("INSERT INTO `{$txt}_elements` (txtver_id,txt_position,parent,position,element_name,element_id,attributes,contents) VALUES ($values)")) {
			$this->_failure("Cannot insert element into database: ".mysql_error());
			return FALSE;
		}
		return mysql_insert_id();
	}
...
#		if (!$dbresult = mysql_query("SELECT id FROM `{$txt}_elements` WHERE parent='$id'"))
#			$this->_fail("Cannot count elements in database: ".mysql_error());
#		if (mysql_num_rows($dbresult))
#			while ($child=mysql_fetch_assoc($dbresult)) $this->delete_subtree($txt,$child['id']);
#		if (!$dbresult = mysql_query("DELETE FROM `{$txt}_elements` WHERE (id='$id')"))
#				$this->_fail("Cannot access database: ".mysql_error());
#		return TRUE;
#	}
...
# delete alignment
	function delete_alignment($aid) {
		...
		if (!$dbresult = mysql_query("DELETE FROM `{$txt}_links` WHERE alignment_id='$aid'"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (!$dbresult = mysql_query("DELETE FROM alignments WHERE id='$aid'"))
			$this->_fail("Cannot access database: ".mysql_error());
		$dbresult = @mysql_query("DELETE FROM `{$txt}_align_changelog` WHERE (alignment_id=$aid)");
		return TRUE;
	}

# Get element by ID from database
	function get_element($txt,$id) {
		if (!$dbresult = mysql_query("SELECT * FROM `{$txt}_elements` WHERE id='$id'"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (mysql_num_rows($dbresult)) {
			$element = mysql_fetch_assoc($dbresult);
			$element['children'] = array();
			if (!$dbresult = mysql_query("SELECT id FROM `{$txt}_elements` WHERE parent='$id' ORDER BY position"))
				$this->_fail("Cannot access database: ".mysql_error());
			while ($child=mysql_fetch_assoc($dbresult)) $element['children'][]=$child['id'];
			return $element;
		} else return;
	}
...
# Return database ID of element with given XML id (and text version id)
	function get_id_by_element_id($eid,$txt,$vid) {
		if (!$dbresult = mysql_query("SELECT * FROM `{$txt}_elements` WHERE (element_id='$eid' AND txtver_id='$vid')"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (mysql_num_rows($dbresult)) {
			$element = mysql_fetch_assoc($dbresult);
			return $element['id'];
...
# Make-up SQL filtering conditions
	function make_filter($filter) {
		...
		if (IsSet($filter['tname']) && $filter['tname']!='')
			$filter_cond .= " AND LOCATE('".mysql_real_escape_string($filter['tname'])."',t.name)>0";
		if (IsSet($filter['v1name']) && $filter['v1name']!='')
			$filter_cond .= " AND LOCATE('".mysql_real_escape_string($filter['v1name'])."',v1.version_name)>0";
		if (IsSet($filter['v2name']) && $filter['v2name']!='')
			$filter_cond .= " AND LOCATE('".mysql_real_escape_string($filter['v2name'])."',v2.version_name)>0";
...
# Get alignments by text version
	function get_alignments($txt,$vid,$order='t.name ASC, v1.version_name ASC',$filter=array()) {
		$ret = array(); $txt = mysql_real_escape_string($txt);
		$filter_cond = $this->make_filter($filter);
		if ($vid>0) $vcond = "AND (v1.id=$vid OR v2.id=$vid)"; else $vcond = '';
		if (!$dbresult = mysql_query("SELECT a.*, t.name as text_name, v1.version_name as v1_name, v2.version_name as v2_name FROM texts t, alignments as a, versions as v1, versions as v2 WHERE t.id=$txt AND v1.text_id=$txt AND v2.text_id=$txt $vcond AND v1.id=a.ver1_id AND v2.id=a.ver2_id$filter_cond ORDER BY $order"))
			$this->_fail("Cannot access database: ".mysql_error());
		while ($ret[] =  mysql_fetch_assoc($dbresult));
		array_pop($ret);
		return $ret;
	}
...
# Get alignment by name
	function get_alignment_by_name($tname,$v1name,$v2name) {
		global $_ERROR;
		if (!$dbresult = mysql_query("SELECT a.* FROM texts t, alignments as a, versions as v1, versions as v2 WHERE t.name='$tname' AND v1.text_id=t.id AND v1.version_name='$v1name' AND v2.text_id=t.id AND v2.version_name='$v2name' AND v1.id=a.ver1_id AND v2.id=a.ver2_id"))
			$this->_fail("Cannot access database: ".mysql_error());
...
# Get alignment's last text change
	function get_document_lastchange($txt, $verid, $userid = -1) {
		...
		if (!$dbresult = mysql_query("SELECT ts FROM `{$txt}_changelog` WHERE txtver_id='$verid' $excl ORDER BY ts DESC LIMIT 1"))
			$this->_fail("Cannot access database: ".mysql_error());
		while ($ret[] =  mysql_fetch_assoc($dbresult));
		array_pop($ret);
		return $ret[0]['ts'];
...
# Get aligned items
	function get_aligned_items($txt,$aid,$pos,$limit=200,$maxstatus=STATUS_PLAIN) {
		...
		if (!$dbresult = mysql_query($query))
			$this->_fail("Cannot access database: ".mysql_error()."\nQuery: ".$query);
		while ($row =  mysql_fetch_assoc($dbresult)) { $ret[$row['link_position']][$row['txtver_id']][] = $row; }
		return $ret;
	}
...
# Get text version id by change_id
	function get_change($aid,$chid) {
		...
		if (!$dbresult = mysql_query($query))
			$this->_fail("Cannot access database: ".mysql_error()."\nQuery: ".$query);
		if ($change = mysql_fetch_assoc($dbresult)) {
			return $change;
		} else return false;
	}
...
# Get first link position with given status
	function get_pos_by_status($aid,$minstatus) {
		$al = $this->alignment_info($aid);
		$txt = $al['text_id'];
		if (!$dbresult = mysql_query("SELECT min(position) as pos FROM `{$txt}_links` WHERE (alignment_id='$aid' AND status>='$minstatus')"))
			$this->_fail("Cannot access database: ".mysql_error());
		$ret = mysql_fetch_assoc($dbresult);
		if ($ret['pos']!='') {
			return $ret['pos'];
...
# Get txt version by link id
	function get_ver_by_linkid($txt,$lid) {
		if (!$dbresult = mysql_query("SELECT version_id FROM `{$txt}_links` WHERE id=$lid"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (mysql_num_rows($dbresult)) {
			$ret = mysql_fetch_assoc($dbresult);
...
# Get info about link
	function get_link_info($txt,$lid) {
		if (!$dbresult = mysql_query("SELECT * FROM `{$txt}_links` WHERE id=$lid"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (mysql_num_rows($dbresult)) {
			$ret = mysql_fetch_assoc($dbresult);
			return $ret;
...
# Get txt version by element id
	function get_ver_by_id($txt,$id) {
		if (!$dbresult = mysql_query("SELECT txtver_id FROM `{$txt}_elements` WHERE id=$id"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (mysql_num_rows($dbresult)) {
			$ret = mysql_fetch_assoc($dbresult);
...
# Decrement position of alignment link
	function alitem_pos_dec($txt,$aid,$id,$pos,$userid=0) {
		...
		if (mysql_num_rows($dbresult)) return;
		# Update
		if (!$dbresult = mysql_query("UPDATE `{$txt}_links` SET position=position-1 WHERE id=$id"))
			$this->_fail("Cannot access database: ".mysql_error());
...
# Move all version links one position down
	function alver_dec($txt,$aid,$ver,$pos,$userid=0) {
		if ($pos==1) return FALSE;
		if (!$dbresult = mysql_query("UPDATE `{$txt}_links` SET position=position-1 WHERE alignment_id=$aid AND version_id=$ver AND position>=$pos"))
			$this->_fail("Cannot access database: ".mysql_error());
		if ($this->auto_status_update) $this->update_manual_status($txt,$aid,$pos-1);
...
# Move all links one position down (both versions)
	function alpos_dec($txt,$aid,$pos,$userid=0) {
		if ($pos==1) return FALSE;
		if (!$dbresult = mysql_query("UPDATE `{$txt}_links` SET position=position-1 WHERE alignment_id=$aid AND position>=$pos"))
			$this->_fail("Cannot access database: ".mysql_error());
		if ($this->auto_status_update) $this->update_manual_status($txt,$aid,$pos-1);
...
# Update status to STATUS_MANUAL up to the given position
	function update_manual_status($txt,$aid,$pos) {
		if (!$dbresult = mysql_query("UPDATE `{$txt}_links` SET status=".STATUS_MANUAL." WHERE alignment_id=$aid AND position<=$pos AND (status=".STATUS_PLAIN." OR status=".STATUS_AUTOMATIC.")"))
			$this->_fail("Cannot access database: ".mysql_error());
	}
...
# Update text of an element
	...
	function _update_element_text($txt,$id,$text,$userid=0,$change='E',$assoc='NULL', $letopen=false) {
		...
		if (mysql_num_rows($dbresult)) $el = mysql_fetch_assoc($dbresult);
		else  {
		...
		# Update...
		if (!$dbresult = mysql_query("UPDATE `{$txt}_elements` SET contents='".mysql_real_escape_string($text)."' WHERE id=$id")) {
			$this->_failure("Cannot access database: ".mysql_error());
			return FALSE;
		}
		# Mark text as changed
		if (!$dbresult = mysql_query("UPDATE versions SET text_changed=TRUE WHERE text_id=$txt AND id={$el['txtver_id']}")) {
			$this->_failure("Cannot access database: ".mysql_error());
			return FALSE;
		}
		return $change_id;
...
# Reset remote changes as closed (fully comitted)
	function close_updates($txt, $verid, $userid) {
		...
		if ($LOG_EDIT_CHANGES==true) {
			if (!$dbresult = mysql_query("UPDATE `{$txt}_changelog` SET open=0 WHERE txtver_id='$verid' AND userid='$userid'"))
				$this->_fail("Cannot access database: ".mysql_error());
			return 1;
		} else return 0;
	}
...
# Log change of text
	function log_edit_change($txt, $version, $element_id, $assoc, $change, $userid, $old_contents, $open=false) {
		...
		if ($LOG_EDIT_CHANGES==true) {
			...
			$values = "$version, $element_id, $assoc, '$change', $userid, '".mysql_real_escape_string($old_contents)."', now(), $openval";
			if (!$dbresult = mysql_query("INSERT INTO `{$txt}_changelog` (txtver_id,element_id,assoc_id,chtype,userid,old_contents,ts,open) VALUES ($values)")) {
				$this->_failure("Cannot access database: ".mysql_error());//$_ERROR.="mysqlid:".mysql_insert_id();
				return FALSE;
			}
			return mysql_insert_id();
		} else return 0;
...
$table_align_changelog = "CREATE TABLE IF NOT EXISTS `{$txt}_align_changelog` (
	...
	INDEX index_ts (ts)
			...
			if (!$dbresult = mysql_query("SET storage_engine=InnoDB"))
				$this->_fail("Cannot set default storage engine to InnoDB: ".mysql_error());
			if (!$dbresult = mysql_query($table_align_changelog))
				$this->_fail("Cannot create new alignment changelog table in the database: ".mysql_error());
			$values = "$aid, $ver, $id, $pos, '$type', $userid, now()";
			if (!$dbresult = mysql_query("INSERT INTO `{$txt}_align_changelog` (alignment_id, txtver_id, link_id, position, chtype, userid, ts) VALUES ($values)"))
				$this->_fail("Cannot insert into alignment changelog table: ".mysql_error());
			return mysql_insert_id();
		} else return 0;
	}
...
# Split element (given id and array of parts (strings))
	function split_element($txt,$id,$parts,$userid=0, $letopen=false) {
		...
		if (!$dbresult = mysql_query("SELECT * FROM `{$txt}_links` WHERE (element_id='{$id}')")) {
			$this->_failure("Cannot access database: ".mysql_error());
			return FALSE;
		}
		$links=array();
		while ($links[] = mysql_fetch_assoc($dbresult));
		# Check for brokem XML elements...
		...
		if (!$dbresult = mysql_query("UPDATE `{$txt}_elements` SET position=position+".count($parts)." WHERE txtver_id={$el['txtver_id']} AND parent={$el['parent']} AND position>$pos")) {
			$this->_failure("Cannot access database: ".mysql_error());
			return FALSE;
		}
		# And shift txt_position of all elements upwards
		if (!$dbresult = mysql_query("UPDATE `{$txt}_elements` SET txt_position=txt_position+".(count($parts)*2)." WHERE txtver_id={$el['txtver_id']} AND txt_position>$txtpos")) {
			$this->_failure("Cannot access database: ".mysql_error());
			return FALSE;
		}
...
# Check possibility of a (multi)merge (for API.php)
	function merge_possible($txt, $excludeaid, $id, $count=1) {
		...
		if (!$dbresult = mysql_query("SELECT * FROM `{$txt}_elements` WHERE (id='{$id}')"))
			$this->_fail("Cannot access database1: ".mysql_error());
		if (mysql_num_rows($dbresult)) $el1 = mysql_fetch_assoc($dbresult);
		...
		foreach($txt_elements as $element) $alignables[]="element_name='$element'";
		$alignable = join(' OR ',$alignables);
		if (!$dbresult = mysql_query("SELECT * FROM `{$txt}_elements` WHERE (txtver_id={$el1['txtver_id']} AND txt_position>'{$el1['txt_position']}' AND ($alignable)) ORDER BY txt_position LIMIT $count"))
			$this->_fail("Cannot access database2: ".mysql_error());
		while ($ret = mysql_fetch_assoc($dbresult)) { $el2 = $ret; };
		# Are the two elements siblings or at least their parents?
		$parental=FALSE;
		if ($el1['parent']!=$el2['parent']) {
			if (!$dbresult = mysql_query("SELECT parent, position FROM `{$txt}_elements` WHERE (id='{$el1['parent']}' OR id='{$el2['parent']}') ORDER BY txt_position"))
				$this->_fail("Cannot access database3a: ".mysql_error());
			while ($p[] = mysql_fetch_assoc($dbresult));
...
# Merge two elements (the given one and the following one)
	function merge_elements($txt,$id,$userid=0, $letopen=false, $exclude_al=-1) {
		...
		# Find out more about the first element we are going to merge
		if (!$dbresult = mysql_query("SELECT * FROM `{$txt}_elements` WHERE (id='{$id}')")) {
			$this->_failure("Cannot access database1: ".mysql_error());
			return FALSE;
		}
		if (mysql_num_rows($dbresult)) $el1 = mysql_fetch_assoc($dbresult);
		else {
		...
		foreach($txt_elements as $element) $alignables[]="element_name='$element'";
		$alignable = join(' OR ',$alignables);
		if (!$dbresult = mysql_query("SELECT * FROM `{$txt}_elements` WHERE (txtver_id={$el1['txtver_id']} AND txt_position>'{$el1['txt_position']}' AND ($alignable)) ORDER BY txt_position LIMIT 1")) {
			$this->_failure("Cannot access database2: ".mysql_error());
			return FALSE;
		}
		if (mysql_num_rows($dbresult)) $el2 = mysql_fetch_assoc($dbresult);
#print $el1['id']." <> ".$el2['id']."\n";
		# Are the two elements siblings or at least their parents?
		$parental=FALSE;
		if ($el1['parent']!=$el2['parent']) {
#print "Different parents!! {$el1['parent']} and {$el1['parent']}\n";
			if (!$dbresult = mysql_query("SELECT parent, position FROM `{$txt}_elements` WHERE (id='{$el1['parent']}' OR id='{$el2['parent']}') ORDER BY txt_position")) {
				$this->_failure("Cannot access database3a: ".mysql_error());
				return FALSE;
			}
			while ($p[] = mysql_fetch_assoc($dbresult));
#print "Parent's parents are {$p[0]['parent']} and {$p[1]['parent']}\n";
		...
		if (!$dbresult = mysql_query("SELECT v1.version_name as v1, v2.version_name as v2, a.id as aid, l1.position as p1, l2.position as p2 FROM `{$txt}_links` as l1, `{$txt}_links` as l2, alignments as a, versions as v1, versions as v2  WHERE ({$excl}l1.element_id={$el1['id']} AND l2.element_id={$el2['id']} AND l1.alignment_id=l2.alignment_id AND l1.position!=l2.position AND a.id=l1.alignment_id AND v1.id=a.ver1_id AND v2.id=a.ver2_id$ignoreunconfirmed)")) {
			$this->_failure("Cannot access database3b: ".mysql_error());
			return FALSE;
		}
#print "P1\n";
		if (mysql_num_rows($dbresult)) {
...
#print "P2\n";
		...
		if (!mysql_query("DELETE FROM `{$txt}_elements` WHERE (txtver_id={$el1['txtver_id']} AND txt_position>'{$el1['txt_position']}' AND txt_position<='{$el2['txt_position']}')")) {
			$this->_failure("Cannot access database4: ".mysql_error());
			return FALSE;
		}
		# Now, remove the links of the deleted element
#print "Removin links...\n";
		if (!mysql_query("DELETE FROM `{$txt}_links` WHERE (element_id={$el2['id']})")) {
			$this->_failure("Cannot access database5: ".mysql_error());
			return FALSE;
		}
		# Decrease the position of the following elements
#print "Decreasing position...\n";
		$diff=$el2['txt_position']-$el1['txt_position'];
		if (!mysql_query("UPDATE `{$txt}_elements` SET txt_position=txt_position-$diff WHERE txtver_id={$el1['txtver_id']} AND txt_position>{$el1['txt_position']}")) {
			$this->_failure("Cannot access database6: ".mysql_error());
			return FALSE;
		}
		# if we deleted the parent of the second node, we need to connect its siblings to the new parent
		if ($parental) {
#print "Reconnecting parent...\n";
			if (!mysql_query("UPDATE `{$txt}_elements` SET parent={$el1['parent']} WHERE parent={$el2['parent']}")) {
				$this->_failure("Cannot access database7: ".mysql_error());
				return FALSE;
			}
			# And update positions of the parent's siblings
#print "Updating parents order...\n";
			if (!mysql_query("SET @cnt=0")) { $this->_failure("Cannot access database: ".mysql_error()); return FALSE; }
			if (!mysql_query("UPDATE `{$txt}_elements` SET position=(@cnt:=@cnt+1) WHERE (txtver_id={$el2['txtver_id']} AND parent={$p[0]['parent']}) ORDER BY txt_position")) {
				$this->_failure("Cannot access database8: ".mysql_error());
				return FALSE;
			}
		}
...
#print "Updating sibling order...\n";
		if (!mysql_query("SET @cnt=0")) { $this->_failure("Cannot access database: ".mysql_error()); return FALSE; }
		if (!mysql_query("UPDATE `{$txt}_elements` SET position=(@cnt:=@cnt+1) WHERE (txtver_id={$el2['txtver_id']} AND parent={$el1['parent']} AND substr(element_name,1,2)!='__') ORDER BY txt_position")) {
			$this->_failure("Cannot access database9: ".mysql_error());
			return FALSE;
		}
		if (!$dbresult = mysql_query("COMMIT")) {
			$this->_failure("Cannot commit transaction: ".mysql_error());
...
# Merge parents (of the given one and the preceding one), i.e. delete parent break at the given element
	function merge_parents($txt,$id,$userid=0, $letopen=false) {
		...
		foreach($txt_elements as $element) $alignables[]="element_name='$element'";
		$alignable = join(' OR ',$alignables);
		if (!$dbresult = mysql_query("SELECT * FROM `{$txt}_elements` WHERE (txtver_id={$el1['txtver_id']} AND txt_position<'{$el1['txt_position']}' AND ($alignable)) ORDER BY txt_position DESC LIMIT 1")) {
			$this->_failure("Cannot access database2: ".mysql_error());
			return FALSE;
		}
		if (mysql_num_rows($dbresult)) $el2 = mysql_fetch_assoc($dbresult);
		...
		if (!$dbresult = mysql_query("SELECT parent, position, txt_position FROM `{$txt}_elements` WHERE (id='{$el1['parent']}' OR id='{$el2['parent']}') ORDER BY txt_position")) {
			$this->_failure("Cannot access database3a: ".mysql_error());
			return FALSE;
		}
		while ($p[] = mysql_fetch_assoc($dbresult));
		...
		if (!mysql_query("DELETE FROM `{$txt}_elements` WHERE id={$el1['parent']}")) {
			$this->_failure("Cannot access database4a: ".mysql_error());
			return FALSE;
		}
		# Delete whitespace
		if (!mysql_query("DELETE FROM `{$txt}_elements` WHERE (parent={$el1['parent']} AND txt_position=({$p[1]['txt_position']}+1) AND element_name='__WS__')")) {
			$this->_failure("Cannot access database4b: ".mysql_error());
			return FALSE;
		}
		# Delete possible elements/whitespace inbetween the parents
		if (!mysql_query("DELETE FROM `{$txt}_elements` WHERE (txtver_id={$el2['txtver_id']} AND parent={$p[1]['parent']} AND txt_position<{$p[1]['txt_position']} AND txt_position>{$p[0]['txt_position']})")) {
			$this->_failure("Cannot access database5: ".mysql_error());
			return FALSE;
		}
		# Decrease the position of the following elements
		if (!mysql_query("UPDATE `{$txt}_elements` SET txt_position=txt_position-1 WHERE txtver_id={$el1['txtver_id']} AND txt_position>{$p[1]['txt_position']}")) {
			$this->_failure("Cannot access database6: ".mysql_error());
			return FALSE;
		}
		# connect elements to the new parent
		if (!mysql_query("UPDATE `{$txt}_elements` SET parent={$el2['parent']} WHERE parent={$el1['parent']}")) {
			$this->_failure("Cannot access database7: ".mysql_error());
			return FALSE;
		}
		# And update positions of the parent's siblings
		...
		# Finally, update relative positions of the new siblings
		if (!mysql_query("SET @cnt=0")) { $this->_failure("Cannot access database: ".mysql_error()); return FALSE; }
		if (!mysql_query("UPDATE `{$txt}_elements` SET position=(@cnt:=@cnt+1) WHERE (txtver_id={$el2['txtver_id']} AND parent={$el2['parent']} AND substr(element_name,1,2)!='__') ORDER BY txt_position")) {
			$this->_failure("Cannot access database9: ".mysql_error());
			return FALSE;
		}
		if (!$dbresult = mysql_query("COMMIT")) {
...
# Splait parent (of the given element just before it), i.e. create new parent break at the given element
	function split_parent($txt,$id,$userid=0, $letopen=false) {
		...
		if (!$dbresult = mysql_query("SELECT * FROM `{$txt}_elements` WHERE (id='{$el1['parent']}')")) {
			$this->_failure("Cannot access database1: ".mysql_error());
			return FALSE;
		}
		while ($p[] = mysql_fetch_assoc($dbresult));
		...
		if (!mysql_query("UPDATE `{$txt}_elements` SET txt_position=txt_position+3 WHERE txtver_id={$el1['txtver_id']} AND txt_position>={$el1['txt_position']}")) {
			$this->_failure("Cannot access database2: ".mysql_error());
			return FALSE;
		}
		...
		# connect elements to the new parent
		if (!mysql_query("UPDATE `{$txt}_elements` SET parent=$newid WHERE parent={$el1['parent']} AND txt_position>=({$el1['txt_position']}+2)")) {
			$this->_failure("Cannot access database3: ".mysql_error());
			return FALSE;
		}
		# And update positions of the parent's siblings
		if (!mysql_query("SET @cnt=0")) { $this->_failure("Cannot access database: ".mysql_error()); return FALSE; }
		if (!mysql_query("UPDATE `{$txt}_elements` SET position=(@cnt:=@cnt+1) WHERE (txtver_id={$el1['txtver_id']} AND parent={$p[0]['parent']}) ORDER BY txt_position")) {
			$this->_failure("Cannot access database4: ".mysql_error());
			return FALSE;
		}
		# Finally, update relative positions of the new siblings
		if (!mysql_query("SET @cnt=0")) { $this->_failure("Cannot access database: ".mysql_error()); return FALSE; }
		if (!mysql_query("UPDATE `{$txt}_elements` SET position=(@cnt:=@cnt+1) WHERE (txtver_id={$el1['txtver_id']} AND parent=$newid AND substr(element_name,1,2)!='__') ORDER BY txt_position")) {
			$this->_failure("Cannot access database5: ".mysql_error());
			return FALSE;
		}
		if (!$dbresult = mysql_query("COMMIT")) {
...
# Get names of elements used as containers for aligned elements
	function get_container_names($txt,$docid) {
		...
		if(!$dbresult = mysql_query($query))
			$this->_fail("Cannot access database (container names): ".mysql_error());
		while ($ret = mysql_fetch_assoc($dbresult)) $names[]=$ret['name'];
		return $names;
	}
...
# Are given elements nested in given document?
	function are_nested($element_names,$txt,$docid) {
		...
		while ($affected>1) {
			mysql_query("ALTER TABLE tmp_nesting RENAME TO tmp_elements") or $this->_fail("Cannot access database: ".mysql_error());
			mysql_query("ALTER TABLE tmp_elements ADD INDEX index_parent (parent)") or $this->_fail("Cannot access database: ".mysql_error());
			...
			mysql_query($query) or $this->_fail("Cannot access database: ".mysql_error());
			$affected=mysql_affected_rows();
			mysql_query("DROP TABLE tmp_elements") or $this->_fail("Cannot access database: ".mysql_error());
			$query = "SELECT element_name, id FROM tmp_nesting WHERE $filter";
			$dbresult=mysql_query($query) or $this->_fail("Cannot access database: ".mysql_error());
			if (mysql_num_rows($dbresult)) {
				mysql_query("DROP TABLE tmp_nesting") or $this->_fail("Cannot access database: ".mysql_error());
				return TRUE;
			}
		}
		mysql_query("DROP TABLE tmp_nesting") or $this->_fail("Cannot drop temp table in database: ".mysql_error());
		return FALSE;
	}
...
# Get text count
  function texts_count($filter=array()) {
    $filter_cond = '';
    if (IsSet($filter['tname']) && $filter['tname']!='') {
      $filter_cond .= "LOCATE('".mysql_real_escape_string($filter['tname'])."',t.name)>0";
    }
    if (IsSet($filter['vname']) && $filter['vname']!='') {
      $vcond .= "LEFT JOIN versions as v ON v.text_id=t.id WHERE LOCATE('".mysql_real_escape_string($filter['vname'])."',v.version_name)>0";
      if ($filter_cond!='') $filter_cond = $vcond.' AND '.$filter_cond;
      else $filter_cond = $vcond;
    } else {
      if ($filter_cond!='') $filter_cond = 'WHERE '.$filter_cond;
    }
    if (!$dbresult = mysql_query("SELECT count(*) as count FROM texts as t $filter_cond"))
      $this->_fail("Cannot access database: ".mysql_error());
    $ret = mysql_fetch_assoc($dbresult);
    return $ret['count'];
  }
...
# List all texts and versions
	function list_texts($offset = 0, $limit = 0, $filter=array()) {
		$texts=array();
// 		if (!$dbresult = mysql_query("SHOW TABLES LIKE '{$txt}_versions'"))
// 			$this->_fail("Cannot access database: ".mysql_error());
// 		while ($ret = mysql_fetch_row($dbresult)) { 
// 			$ret = array_shift($ret);
// 			$txt = mysql_real_escape_string( substr($ret,0,strlen($ret)-9) );
// 			$query = "SELECT '$txt' as text_id, '$txt' as text_name, v.id as version_id, v.version_name as version_name, v.root_id as root_id, v.text_changed as text_changed, v.uniq_ids as uniq_ids, (SELECT count(*) FROM `{$txt}_alignments` WHERE ver1_id=v.id OR ver2_id=v.id) as alignment_count FROM `{$txt}_versions` as v ORDER BY v.version_name";
// 			if (!$dbresult1 = mysql_query($query))
// 				$this->_fail("Cannot access database: ".mysql_error());
// 			while ($ret1 = mysql_fetch_assoc($dbresult1)) { $texts[] = $ret1; };
// 		}
    $filter_cond = '';
    if (IsSet($filter['tname']) && $filter['tname']!='') {
      $filter_cond .= "LOCATE('".mysql_real_escape_string($filter['tname'])."',t.name)>0";
    ...
    if ($limit || $filter_cond!='') {
      ...
      if (!$dbresult1 = mysql_query($pquery))
        $this->_fail("Cannot access database: ".mysql_error());
      if (mysql_num_rows($dbresult1)) {
		$tids = array();
		while ($ret1 = mysql_fetch_assoc($dbresult1)) { $tids[] = $ret1['id']; };
	...
	function update_eids($txt,$docid,$twolevel='') {
		...
		if ($twolevel) {
			...
			if (!$this->are_nested($containers,$txt,$docid)) {
				...
				@mysql_query("SET @num:=0") or $this->_fail("Cannot initiate counter in database: ".mysql_error());
				@mysql_query("UPDATE `{$txt}_elements` SET element_id=@num:=@num+1 WHERE txtver_id=$docid AND ($contel) ORDER BY txt_position") 
					or $this->_fail("Cannot update parent elements in database: ".mysql_error());
				# Now, copy the parent's (container's) new IDs to the alignable elements
				@mysql_query("UPDATE `{$txt}_elements` e, `{$txt}_elements` p SET e.element_id=p.element_id WHERE e.txtver_id=$docid AND e.parent=p.id AND ($alignable)")
					or $this->_fail("Cannot update elements in database (eid update1): ".mysql_error());
				# Finally, update the ID's of the alignable elements
				@mysql_query("SET @num:=0, @prp:=0") or $this->_fail("Cannot initiate counters in database: ".mysql_error());
				@mysql_query("UPDATE `{$txt}_elements` e SET element_id=CONCAT(element_id,':',@num:=if((@prp+0)=(parent+0),@num+1,1)), parent=@prp:=parent WHERE txtver_id=$docid AND ($alignable) ORDER BY txt_position")
					or $this->_fail("Cannot update elements in database (eid update2): ".mysql_error());
		...
		@mysql_query("SET @num=0") or $this->_fail("Cannot initiate counter in database: ".mysql_error());
		@mysql_query("UPDATE `{$txt}_elements` e SET element_id=@num:=@num+1 WHERE txtver_id=$docid AND ($alignable) ORDER BY txt_position")
			or $this->_fail("Cannot update elements: ".mysql_error());
		$this->set_uniq_ids($txt,$docid,true);
		return 1;
	}
...
# Set the "uniq_ids" flag
	function set_uniq_ids($txt,$vid,$value) {
		...
		if (!@mysql_query("UPDATE versions SET uniq_ids=$value WHERE text_id=$txt AND id=$vid")) {
			$this->_failure("Cannot update: ".mysql_error());
			return FALSE;
		}
		return TRUE;
	}
...
# Change status of a single link
	function change_status($aid,$pos,$status) {
		$al = $this->alignment_info($aid);
		$txt = $al['text_id'];
		@mysql_query("UPDATE `{$txt}_links` SET status=$status WHERE alignment_id=$aid AND position=$pos")
			or $this->_fail("Cannot update link: ".mysql_error());
		return TRUE;
	}
...
# Change status of a single link
	function change_mark($aid,$pos,$val) {
		$al = $this->alignment_info($aid);
		$txt = $al['text_id'];
		@mysql_query("UPDATE `{$txt}_links` SET mark=$val WHERE alignment_id=$aid AND position=$pos")
			or $this->_fail("Cannot update link: ".mysql_error());
		return TRUE;
	}
...
# HUNALIGN alignment
    ...
    if (!$dbresult = mysql_query("START TRANSACTION")) {
      ...
      return FALSE;
		...
		if (!$dbresult = mysql_query("SELECT id,element_name,contents,element_id FROM `{$txt}_elements` WHERE (txtver_id='$v1' AND ($alignable1) AND id NOT IN (SELECT element_id FROM `{$txt}_links` WHERE alignment_id='$aid' AND version_id='$v1')) ORDER BY txt_position")) {
			$this->_failure("Cannot access database: ".mysql_error());
      if ($report) print("$_ERROR\n");
      return FALSE;
    }
		...
		# Get unaligned elements from text version 2
		if (!$dbresult = mysql_query("SELECT id,element_name,contents,element_id FROM `{$txt}_elements` WHERE (txtver_id='$v2' AND ($alignable2) AND id NOT IN (SELECT element_id FROM `{$txt}_links` WHERE alignment_id='$aid' AND version_id='$v2')) ORDER BY txt_position")) {
      $this->_failure("Cannot access database: ".mysql_error());
      if ($report) print("$_ERROR\n");
      return FALSE;
    }
  ...
  return $uid;
	...
	function move_to($txt,$lid,$newpos) {
		...
		if ($newpos<$oldpos) {
			...
			if (!$dbresult = mysql_query("SELECT id FROM `{$txt}_links` WHERE alignment_id=$aid AND version_id=$ver AND position>$newpos AND position<$oldpos"))
				$this->_fail("Cannot access database1: ".mysql_error());
			if (mysql_num_rows($dbresult)) {
				return "Cannot move the text. There are other elements between position $newpos and $oldpos. This range must be empty.";
			}
			$diff = $oldpos-$newpos;
			if (!$dbresult = mysql_query("UPDATE `{$txt}_links` SET position=position-$diff WHERE alignment_id=$aid AND version_id=$ver AND position>=$oldpos"))
			$this->_fail("Cannot access database: ".mysql_error());
...
# Delete links from the given position up (for re-alignment)
	function delete_alignment_from_pos($aid,$pos) {
		$al = $this->alignment_info($aid);
		$txt = $al['text_id'];
		if (!$dbresult = mysql_query("DELETE FROM `{$txt}_links` WHERE alignment_id=$aid AND position>=$pos"))
			$this->_fail("Cannot access database: ".mysql_error());
		return true;
	}
...
# Change the "editor" for alignment
	function alignment_cheditor($aid,$userid) {
		...
		@mysql_query("UPDATE alignments SET editor=$userid WHERE id=$aid")
			or $this->_fail("Cannot update alignments: ".mysql_error());
		return true;
	}
...
# Change the "resp" for alignment
	function alignment_chresp($aid,$userid) {
		...
		@mysql_query("UPDATE alignments SET resp=$userid WHERE id=$aid")
			or $this->_fail("Cannot update alignments: ".mysql_error());
		return true;
	}
...
# Change the "remote_user" for alignment
	function alignment_chruser($aid,$userid) {
		if ($userid=='') return FALSE;
		@mysql_query("UPDATE alignments SET remote_user=$userid WHERE id=$aid")
			or $this->_fail("Cannot update alignments: ".mysql_error());
		return true;
	}
...
# Change the "status" for alignment
	function alignment_chstat($aid,$status) {
		...
		if ($retval==0) {
			@mysql_query("UPDATE alignments SET status=$status WHERE id=$aid")
			or $this->_fail("Cannot update alignments: ".mysql_error());
			if ($status==ALSTAT_REMOTE) {
				@mysql_query("UPDATE alignments SET remote_user=editor WHERE id=$aid")
				or $this->_fail("Cannot update alignments: ".mysql_error());
			}
			if ($ret!='') {
				$_ERROR = "Warning: ".$ret;
...
# Lock alignment for a remote editor
	function alignment_remote_lock($aid,$userid) {
		@mysql_query("UPDATE alignments SET status=".ALSTAT_REMOTE.", remote_user=$userid WHERE id=$aid")
			or $this->_fail("Cannot update alignments: ".mysql_error());
	}

# Unlock alignment
	function alignment_remote_unlock($aid,$userid,$stat) {
		@mysql_query("UPDATE alignments SET status=$stat, remote_user=NULL WHERE id=$aid")
			or $this->_fail("Cannot update alignments: ".mysql_error());
	}
...
# Check whether the alignment is complete
	function alignment_check_complete($aid) {
		...
		if (!$dbresult = mysql_query("SELECT id,element_name,contents FROM `{$txt}_elements` WHERE (txtver_id='$v1' AND ($alignable1) AND id NOT IN (SELECT element_id FROM `{$txt}_links` WHERE alignment_id='$aid' AND version_id='$v1')) ORDER BY txt_position"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (mysql_num_rows($dbresult)) {
		...
		if (!$dbresult = mysql_query("SELECT id,element_name,contents FROM `{$txt}_elements` WHERE (txtver_id='$v2' AND ($alignable2) AND id NOT IN (SELECT element_id FROM `{$txt}_links` WHERE alignment_id='$aid' AND version_id='$v2')) ORDER BY txt_position"))
			$this->_fail("Cannot access database: ".mysql_error());
		if (mysql_num_rows($dbresult)) {
			$_ERROR = "Error: The alignment is incomplete. Re-align it and finish it.";
			return FALSE;
...
# Change the "chtext" for alignment
	function alignment_chtext($aid,$value) {
		if ($value) $value='TRUE'; else $value='FALSE';
		@mysql_query("UPDATE alignments SET chtext=$value WHERE id=$aid")
			or $this->_fail("Cannot update alignments: ".mysql_error());
		return true;
	}
...
# Change the "c_chstruct" for alignment
	function alignment_chcstruct($aid,$value) {
		if ($value) $value='TRUE'; else $value='FALSE';
		@mysql_query("UPDATE alignments SET c_chstruct=$value WHERE id=$aid")
			or $this->_fail("Cannot update alignments: ".mysql_error());
		return true;
	}
...
# Swap text versions in alignment
	function swap_versions($aid) {
		...
		if (!$dbresult = mysql_query("UPDATE alignments SET ver1_id={$al['ver2_id']},ver2_id={$al['ver1_id']} WHERE (id=$aid)"))
			$this->_fail("Cannot access database: ".mysql_error());
		return true;
	}
...
# Delete user
	function user_delete($id) {
		...
		if (!$dbresult = mysql_query("DELETE FROM $U_DATABASE.$U_TABLE WHERE $U_USERID='$id'"))
			$this->_fail("Cannot access database: ".mysql_error());
		return true;
	}
